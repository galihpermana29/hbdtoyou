...
export function parsePromptContent(htmlContent: string): ParsedPrompt | null {
  try {
    ...
    if (!htmlContent.includes('Main Story &amp; Vibe:') && !htmlContent.includes('Main Story & Vibe:')) {
      message.error('Missing "Main Story & Vibe:" section. Please include this required section.');
      return null;
    }

    if (!htmlContent.includes('Specific Page Instructions:')) {
      message.error('Missing "Specific Page Instructions:" section. Please include this required section.');
      return null;
    }

    // Parse main theme
    let mainTheme = '';
    const mainStoryRegex = /<h4>Main Story &(?:amp;)? Vibe:<\/h4>([\s\S]*?)(?=<h4>|$)/;
    ...
    if (!mainTheme) {
      message.error('Main Story & Vibe section is empty. Please provide content for this section.');
      return null;
    }

    // Parse specific instructions
    const specificInstructions: Array<{ page: number; texts: string[] }> = [];
    const instructionsRegex = /<h4>Specific Page Instructions:<\/h4>([\s\S]*?)(?=<p><em>|$)/;
    ...
    if (instructionsMatch && instructionsMatch[1]) {
      ...
      const pageRegex = /<p>(?:<strong>)?Page (\d+):?(?:<\/strong>)?<\/p><ul>([\s\S]*?)<\/ul>/g;
      let pageMatch;
...
export function validatePromptStructure(htmlContent: string): boolean {
  const hasMainStory = htmlContent.includes('Main Story &amp; Vibe:') || htmlContent.includes('Main Story & Vibe:');

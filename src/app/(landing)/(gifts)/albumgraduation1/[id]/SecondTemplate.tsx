    ...
    <div className="w-[1080px] mx-auto h-[1920px] bg-black overflow-hidden relative">
      <div className="w-full relative">
        <div className="relative w-full">
          <Image
            src={data?.images?.[0] || '/instagram-template-example.svg'}
            width="0"
            className="w-full object-cover"
            height={1041}
            alt="Instagram Template Example"
          />
          <div
        ...
        <div className="flex items-center justify-between p-6 absolute top-0 left-0 right-0">
          <div className="flex items-center gap-x-3">
            <Image
              ...
              className="mr-5"
            />
            <p className="text-white geist-font text-[32px] leading-5">
              DOCUMENTER
            </p>
          </div>
        ...
        <div className="flex items-center justify-between max-w-[713px] w-full absolute top-[132px] left-1/2 right-1/2 -translate-x-1/2">
          <p className="text-white geist-font text-[32px] leading-[120%] font-normal uppercase">
            Tv Shows
          </p>
          <p className="text-white geist-font text-[32px] leading-[120%] font-normal uppercase">
            Movies
        ...
        <div className="flex flex-col justify-center w-full absolute gap-y-[35px] px-12 py-6 top-[794px]">
          <div className="flex flex-col items-center justify-center gap-y-5">
            <div className="flex flex-col gap-y-2 text-center">
              <p className="geist-font font-bold text-[64px] leading-[120%] text-white">
                {data?.llm_generated?.name}
              </p>
              <div className="flex items-center justify-center gap-x-1">
                <p className="geist-font font-normal text-[28px] leading-[18px] text-white">
                  Thriller
                </p>
                <p className="geist-font font-normal text-[28px] leading-[18px] text-white">
                  •
          ...
          <div className="flex items-center justify-center gap-x-4">
            <p className="geist-font font-normal text-2xl text-[#A4A4A4]">
            ...
            <div className="flex items-center gap-x-2 p-0.5 rounded-md bg-[#525252] border-solid border border-[#838383] h-fit w-fit">
              ...
              <CameraEnhanceIcon sx={{ fontSize: 72, color: 'white' }} />
            </div>
            <p className="geist-font font-semibold text-2xl text-white">
              {data?.llm_generated?.photographer}
            </p>
          </div>
        </div>
      </div>
      <div className="flex flex-col gap-y-8 absolute top-[1237px]">
        <p className="geist-font font-bold text-[32px] leading-[28.9px] tracking-[-0.36px] text-white px-12">
          Top 10 Gallery Today
        </p>
        <div className="flex items-center gap-x-[5.79px] pl-6 w-fit">
          {data?.images?.map((image: string, index: number) => (
            <div
              className="relative w-[245px] h-[230px] flex justify-end items-end"
              key={index}>
              {/* Number first in DOM order so it appears behind the image */}
              <span
                className="geist-font text-[196px] font-extrabold absolute bottom-0 leading-[159.2px] tracking-[-0.36px] left-3 text-transparent"
      ...
      <div className="flex flex-col gap-y-8 absolute top-[1602px]">
        ...
        <div className="flex items-center gap-x-6 pl-12 w-fit">
          {data?.images?.map((image: string, index: number) => (
            <div
              className="w-[235px] h-[368px] relative bg-[#232323] overflow-hidden rounded-xl"
              key={`gallery-${index}`}>

...
import imageFrame from '@/assets/scrapbook2/page3.png';
import texture from '@/assets/scrapbook2/cover3.png';
import { downloadCanvasAsImage } from '../scrapbook/utils/scrapbookUtils';

interface PageTwoVintageBookProps {
  userImage?: string;
...
const PageTwoVintageBook = ({
  userImage,
  canvasRef: externalCanvasRef,
}: PageTwoVintageBookProps) => {
  ...
  // Define frame dimensions and positions - using full canvas for freedom of movement
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (!hasLeftImage) return;

    const rect = canvas.getBoundingClientRect();
    const x = (e.clientX - rect.left) * (canvas.width / rect.width);
    const y = (e.clientY - rect.top) * (canvas.height / rect.height);

    // Allow dragging anywhere on the canvas as long as we have an image
    if (hasLeftImage) {
  ...
  // Helper function to draw an image with freedom of movement across the canvas
  const drawImageInFrame = (
    ...
    // Set base dimensions for the image (use a reasonable size if no frame constraints)
    const baseWidth = frame.width > 0 ? frame.width : ctx.canvas.width * 0.8;
    const baseHeight =
      frame.height > 0 ? frame.height : ctx.canvas.height * 0.8;
    ...
    if (imgRatio > 1) {
      // Landscape image
      drawWidth = baseWidth * position.scale;
      drawHeight = drawWidth / imgRatio;
    } else {
      // Portrait or square image
      drawHeight = baseHeight * position.scale;
      drawWidth = drawHeight * imgRatio;
    }

    // Position is now completely free - use the position values directly
    ...
    // Draw a highlight border when dragging
    if (activeDragFrame === 0 && isDragging) {
  ...
  // Since we only need one image, check if it's uploaded
  const imageUploaded = leftImage !== null;
        ...
        <div className="relative mb-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-1 gap-6">
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              <h3 className="font-medium mb-3">1st Frame</h3>
              {leftImage ? (
      ...
      {leftImage && !cropMode && (
        <div className="mt-4 text-sm text-gray-600 max-w-md">
          ...
          <ul className="list-disc pl-5 mt-1">
            <li>Click and drag anywhere on the canvas to move your image</li>
...
export default PageTwoVintageBook;

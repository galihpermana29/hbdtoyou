...
import { EASE, fadeUp, fadeUpCenter, staggerContainer } from '@/components/wedding/wedding-template-1/variants';
...
function FilmStrip({ content }: { content: WeddingTemplate1Content }) {
  ...
  const photos = [0, 1, 2].map((i) =>
    pickPhoto(content.loveStoryPhotos, i, `${ASSET}/lovestory-photo-${i + 1}.png`)
  );
...
function Polaroid({ content }: { content: WeddingTemplate1Content }) {
  ...
  const isRevealed = !recipientMode || revealed;
  const polaroidPhoto =
